# Sıranın Üstündeki Kalemler

**Seri:** Bilgisayar Mimarisi Serisi — Alt Seri 3: Buyrukların Dünyası — Kitap 3.2
**Yaş grubu:** 9–12 (okuma bilen)
**Ana tema:** Yazmaçlar (hızlı geçici saklama alanları) ve bellek düzeni / bayt sırası
**Karakterler:** Bilge (8 yaşında, kıvırcık kahverengi saçlı, yuvarlak gözlüklü kız), Yonga (küçük yuvarlak mavi-gümüş robot)

---

## Sayfa 1 — Sınıftaki Sıra Çantası

**Metin:**
{B}"Yonga, bilgisayar sayıları nerede tutuyor?
Toplama yaparken rakamları nereye koyuyor?" dedi Bilge.

Yonga güldü: {Y}"Çantanı düşün! Derste her şeyi çantadan çıkarıp çantaya koyamazsın.
Sık kullandığın kalemleri sıranın üstüne koyarsın.
İşlemcinin de böyle hızlı bir 'sıra üstü' alanı var!"

**Resim:**
Horizontal children's book illustration, watercolor style, edge-to-edge, NO white border, NO frame. A cheerful classroom scene. Bilge sits at a wooden school desk with some pencils and a notebook right on top of the desk (easy reach), and a big backpack on the floor beside her. Yonga — a small chubby rounded robot, light blue-silver metallic body, spiral/swirl on belly, big round blue eyes, dome head, short stubby arms, NO TEXT on body, NO antenna — stands on the desk pointing at the pencils vs. the backpack to show the contrast. ONLY ONE ROBOT IN THE SCENE. Warm classroom colors.

---

## Sayfa 2 — Yazmaç Nedir?

**Metin:**
{Y}"Bu hızlı kutulara **yazmaç** (register) denir." dedi Yonga.

{Y}"Yazmaçlar işlemcinin tam içindedir.
Bellekten yüzlerce kat daha hızlıdırlar.
Ama çok küçüktürler: sadece bir sayı sığar her birine."

{B}"Kaç tane var?" diye sordu Bilge.

{Y}"Her işlemcide sayı değişir." dedi Yonga. {Y}"RISC-V'te tam olarak **32 tane!**"

**Resim:**
Horizontal children's book illustration, watercolor style, edge-to-edge, NO white border, NO frame. Inside a stylized cartoon processor chip, 32 small glowing colored boxes are arranged in a neat 4×8 grid, each labeled with a small number (x0 through x31). Each box has a tiny number inside it. Yonga — a small chubby rounded robot, light blue-silver metallic body, spiral/swirl on belly, big round blue eyes, dome head, short stubby arms, NO TEXT on body, NO antenna — stands in front of the grid with arms spread wide proudly. Bilge peers in through a magnifying glass from outside. ONLY ONE ROBOT IN THE SCENE. Electric blues and greens.

---

## Sayfa 3 — Otuz İki Dolap

**Metin:**
Yonga yazmaçların görevlerini saydı:

- **x0**, adı **sıfır**: her zaman 0'dır, hiç değişmez.
- **x1**, adı **ra**: dönüş adresini tutar.
- **x2**, adı **sp**: yığıtın tepesini gösterir.
- **x5–x7**, adları **t0–t2**: geçici değerler için.
- **x10–x17**, adları **a0–a7**: fonksiyona verilen girdiler için.

{Y}"Hepsinin bir adı ve görevi var." dedi Yonga.
{Y}"Bir sınıf gibi: her öğrencinin sırası ve görevi!"

**Resim:**
Horizontal children's book illustration, watercolor style, edge-to-edge, NO white border, NO frame. A colorful school classroom with 32 numbered desks arranged in rows. Each desk has a small name tag. The front desk (x0) is labeled "Always Zero" and has a big 0 on it. Some desks have toy objects on them representing their purpose (a stack of blocks for sp, an arrow for ra). Bilge stands at the front pointing at the desks like a teacher. Yonga — a small chubby rounded robot, light blue-silver metallic body, spiral/swirl on belly, big round blue eyes, dome head, short stubby arms, NO TEXT on body, NO antenna — sits in one of the desks as a student. ONLY ONE ROBOT IN THE SCENE. Bright, cheerful classroom scene.

---

## Sayfa 4 — Sıfır Yazmacı

**Metin:**
{B}"En ilginç yazmaç hangisi?" diye sordu Bilge.

{Y}"x0!" dedi Yonga. {Y}"İçine ne yazarsan yaz, hep sıfır kalır.
Asla değişmez. Silinemez. Her zaman sıfırdır."

{B}"Ne işe yarar ki?" dedi Bilge.

{Y}"Çok işe yarar! Bazen 'bir yazmacı sıfırla' ya da 'sıfırla karşılaştır' gibi
işlemlere ihtiyaç duyarsın. x0 her zaman orada, hazır bekler."

{B}"Çok güçlü bir sıfır!" dedi Bilge.

**Resim:**
Horizontal children's book illustration, watercolor style, edge-to-edge, NO white border, NO frame. One special glowing desk (x0) sits at the very front of the class, surrounded by a golden glow. On the desk, a big bold "0" shines brightly. A cartoon pencil tries to write something else on it, but the number magically changes back to 0 with a sparkle effect. Bilge laughs in surprise. Yonga — a small chubby rounded robot, light blue-silver metallic body, spiral/swirl on belly, big round blue eyes, dome head, short stubby arms, NO TEXT on body, NO antenna — demonstrates the magic with a wand. ONLY ONE ROBOT IN THE SCENE. Magical gold and white tones.

---

## Sayfa 5 — Dev Bir Raf

**Metin:**
{B}"Peki ya normal bellek?" diye sordu Bilge.
{B}"Yazmaçlar yetmez mi?"

{Y}"32 kutu çok az!" dedi Yonga. {Y}"Büyük programlar milyarlarca sayı saklar.
Bunun için dev bir raf var: **bellek** (ya da RAM).
Her raf gözünün bir **adresi** var, tıpkı sokak numarası gibi."

{B}"Raf gözü ne kadar büyük?" diye sordu Bilge.

{Y}"Her gözde tam 8 bit sığar. Buna **bayt** denir."

**Resim:**
Horizontal children's book illustration, watercolor style, edge-to-edge, NO white border, NO frame. A giant warehouse stretching into the distance, with shelves going up and up. Each shelf compartment has a small address number on it (0, 1, 2, 3… continuing far into the background). Each compartment holds exactly 8 small colored balls (representing 8 bits = 1 byte). Bilge stands at the entrance looking amazed at the size. Yonga — a small chubby rounded robot, light blue-silver metallic body, spiral/swirl on belly, big round blue eyes, dome head, short stubby arms, NO TEXT on body, NO antenna — holds a map of the warehouse. ONLY ONE ROBOT IN THE SCENE. Warm warehouse lighting, golden shelves.

---

## Sayfa 6 — Büyük Sayıları Saklamak

**Metin:**
{Y}"Ama bir bayt sadece 0'dan 255'e kadar sayıyı saklar." dedi Yonga.
{Y}"Daha büyük sayılar için birden fazla bayt kullanırsın.
RISC-V'te sayılar genellikle **4 bayt** (32 bit) ya da **8 bayt** (64 bit) kaplar."

{B}"Demek 4 komşu raf gözü birleşip büyük bir sayıyı saklıyor." dedi Bilge.

{Y}"Harika düşündün! Peki hangisi öne, hangisi arkaya gider?
İşte sürpriz burada başlıyor…"

**Resim:**
Horizontal children's book illustration, watercolor style, edge-to-edge, NO white border, NO frame. Four adjacent warehouse compartments (addresses 100, 101, 102, 103) are highlighted. A large colorful number is being split into 4 pieces and each piece placed in a compartment. Bilge looks puzzled about which piece goes first. Yonga — a small chubby rounded robot, light blue-silver metallic body, spiral/swirl on belly, big round blue eyes, dome head, short stubby arms, NO TEXT on body, NO antenna — holds two pieces in its hands as if deciding. ONLY ONE ROBOT IN THE SCENE. Puzzle-like feel, warm colors.

---

## Sayfa 7 — Büyüğü Başta, Küçüğü Başta

**Metin:**
{Y}"Dört gözün hepsi aynı büyüklükte, ama parçaların **değeri** farklı." dedi Yonga. {Y}"385'teki 3 üç yüz demek, 5 ise sadece beş."

{B}"İkisi de tek basamak ama biri daha değerli!" dedi Bilge.

{Y}"İşte soru bu: hangisi öne gitsin? Değerlisi öne giderse **büyüğü başta**, küçüğü öne giderse **küçüğü başta** deriz. İngilizcesi big-endian ve little-endian."

{B}"Ne tuhaf adlar!" dedi Bilge. {B}"Nereden geliyor?"

{Y}"Güliver'in Gezileri adlı kitaptaki yumurta kavgasından! Yumurtayı büyük ucundan mı kır, küçük ucundan mı?"

**Resim:**
Horizontal children's book illustration, watercolor style, edge-to-edge, NO white border, NO frame. Two cartoon characters face each other in a playful argument. Character A (Big-Endian) cracks a large egg from the big end, proudly. Character B (Little-Endian) cracks the same egg from the small pointy end. Between them, four shelf compartments show the same number stored in opposite orders. Bilge watches with a big grin. Yonga — a small chubby rounded robot, light blue-silver metallic body, spiral/swirl on belly, big round blue eyes, dome head, short stubby arms, NO TEXT on body, NO antenna — referees the argument with a small flag. ONLY ONE ROBOT IN THE SCENE. Comic-book style, bright fun colors.

---

## Sayfa 8 — RISC-V'te Küçüğü Başta

**Metin:**
{Y}"RISC-V küçüğü başta düzenini kullanır." dedi Yonga.
{Y}"Demek ki değeri en küçük olan bayt, en düşük adrese gidiyor."

{B}"Bunu aklımda nasıl tutayım?" diye sordu Bilge.

{Y}"Küçüğü başta = değeri en küçük parça önce!" dedi Yonga.
{Y}"Telefonlar, bilgisayarlar, oyun konsolları gibi çoğu günümüz cihazı
küçüğü başta düzenini kullanır."

{B}"Demek yumurta kavgasını küçük-ucular kazandı." dedi Bilge gülerek.

**Resim:**
Horizontal children's book illustration, watercolor style, edge-to-edge, NO white border, NO frame. A podium scene: a small egg stands proudly on the first-place podium, a large egg stands on the second-place podium. Below the first-place podium, four memory compartments show bytes in little-endian order (LSB first). A trophy engraved with a small computer chip sits on the first-place stand. Bilge cheers. Yonga — a small chubby rounded robot, light blue-silver metallic body, spiral/swirl on belly, big round blue eyes, dome head, short stubby arms, NO TEXT on body, NO antenna — hands over the trophy. ONLY ONE ROBOT IN THE SCENE. Playful competition colors, gold and blue.

---

## Sayfa 9 — Yazmaç mı, Bellek mi?

**Metin:**
{B}"Şimdi anlıyorum." dedi Bilge. {B}"Yazmaçlar çok hızlı ama az.
Bellek çok büyük ama daha yavaş."

{Y}"Tam doğru!" dedi Yonga. {Y}"İşlemci önce yazmaçlara bakar.
Orada yoksa bellekten alır.
Bellekten almak yazmaçtan almaktan yüzlerce kat daha uzun sürer."

{B}"Bu yüzden yazmaçları akıllıca kullanmak önemli." dedi Bilge.

{Y}"Doğru bildin! Buna **yazmaç ataması** denir. Programı hazırlayan araçların yaptığı zor bir iştir."

**Resim:**
Horizontal children's book illustration, watercolor style, edge-to-edge, NO white border, NO frame. Side-by-side comparison: Left side shows 32 tiny bright glowing boxes (registers) right next to a small CPU character — close, fast, easy. Right side shows the giant warehouse (RAM) far away in the distance — big but far. A cartoon race shows the CPU reaching registers instantly but having to run a long path to get to RAM. Bilge watches the race. Yonga — a small chubby rounded robot, light blue-silver metallic body, spiral/swirl on belly, big round blue eyes, dome head, short stubby arms, NO TEXT on body, NO antenna — stands at the finish line timing the race. ONLY ONE ROBOT IN THE SCENE. Racing colors, dynamic energy.

---

## Sayfa 10 — Yığıt İşaretçisi

**Metin:**
{Y}"Yığıtın tepesini gösteren yazmacı hatırlıyor musun?" dedi Yonga. {Y}"x2, kısa adıyla **sp**. Yığıt (stack) bir kule gibidir: yalnız en üstüne koyar, yalnız en üstünden alırsın. **En son koyduğun, ilk aldığın olur.**"

{B}"Plastik tabak kuleleri gibi!" dedi Bilge. {B}"Alttakini almak için üstündekileri kaldırman gerekir."

{Y}"Aynen! Kuleye bir tabak koysan da alsan da **sp** hep en üstü gösterir."

**Resim:**
Horizontal children's book illustration, watercolor style, edge-to-edge, NO white border, NO frame. A colorful stack of plastic plates (like in a cafeteria). Each plate is labeled with a piece of data. An arrow labeled "sp" points to the very top plate. In a sequence on the side: (1) a new plate being placed on top (sp moves down), (2) the top plate being removed (sp moves up). Bilge acts out the scenario with plates. Yonga — a small chubby rounded robot, light blue-silver metallic body, spiral/swirl on belly, big round blue eyes, dome head, short stubby arms, NO TEXT on body, NO antenna — demonstrates the sp arrow moving. ONLY ONE ROBOT IN THE SCENE. Cafeteria scene, warm food-service colors.

---

## Sayfa 11 — Bilge Dener

**Metin:**
Bilge bir kağıda yazdı:

{B}"Diyelim ki x5 yazmacında 10, x6 yazmacında 7 var.
Bunları toplamak için işlemci ikisini alır, toplar,
sonucu x7 yazmacına yazar."

{Y}"Aferin!" dedi Yonga. {Y}"Ve bu işlemin hiçbir adımında
bellekten veri almak gerekmedi. Her şey hızlı yazmaçlarda oldu!"

**Resim:**
Horizontal children's book illustration, watercolor style, edge-to-edge, NO white border, NO frame. Three illuminated register boxes labeled x5 (showing "10"), x6 (showing "7"), and x7 (showing "?"). A cartoon addition machine in the center takes glowing balls from x5 and x6, combines them, and shoots the result into x7 which now shows "17". Bilge points at x7 with a big smile. Yonga — a small chubby rounded robot, light blue-silver metallic body, spiral/swirl on belly, big round blue eyes, dome head, short stubby arms, NO TEXT on body, NO antenna — operates the addition machine. ONLY ONE ROBOT IN THE SCENE. Bright, mathematic fun colors.

---

## Sayfa 12 — Küçük ama Önemli

**Metin:**
O akşam Bilge annesiyle yemek masasındayken tuzluğu tutmak istedi.
Uzandı, aldı, koydu. Saniyeler içinde.

{B}"Tuzluk yazmaç gibi." diye düşündü. {B}"Tam yanımda, çok hızlı."

Depo ise çok büyük ama uzakta. Oraya gidip gelmek zaman alırdı.

{B}"Bilgisayar da benim gibi düşünüyor." dedi fısıltıyla gülümseyerek.

**Resim:**
Horizontal children's book illustration, watercolor style, edge-to-edge, NO white border, NO frame. A family dinner table scene. Bilge sits at the table and easily reaches the salt shaker right in front of her (labeled in a thought bubble as "Register — fast & close"). In her imagination bubble, a faraway storage room down a long hallway is labeled "RAM — far & slow." Warm dinner lighting, cozy family scene. Yonga — a small chubby rounded robot, light blue-silver metallic body, spiral/swirl on belly, big round blue eyes, dome head, short stubby arms, NO TEXT on body, NO antenna — peeks around the corner from the hallway smiling. ONLY ONE ROBOT IN THE SCENE. Warm golden dinner colors.

---

## Sayfa 13 — Hatırla!

**Metin:**
Bilge yatmadan önce defterine yazdı:

- **Yazmaç:** işlemci içindeki çok hızlı kutu. RISC-V'te 32 tane (x0–x31).
- **Bellek:** devasa bir raf; her göz 1 bayt.
- **Küçüğü başta:** değeri en küçük bayt önce.
- **Yığıt:** plastik tabak kulesi; sp hep en üstte.

{B}"Yonga, yavaş yavaş anlıyor gibiyim." dedi Bilge.

{Y}"Anlıyor gibi değilsin." dedi Yonga sakin bir sesle. {Y}"Gerçekten anlıyorsun."

**Resim:**
Horizontal children's book illustration, watercolor style, edge-to-edge, NO white border, NO frame. Bilge sits in bed with a notebook on her lap, writing by the warm light of a bedside lamp. The notebook shows her hand-drawn diagrams of register boxes and a memory shelf. Yonga — a small chubby rounded robot, light blue-silver metallic body, spiral/swirl on belly, big round blue eyes, dome head, short stubby arms, NO TEXT on body, NO antenna — sits on the pillow beside her, reading along gently. ONLY ONE ROBOT IN THE SCENE. Cozy night scene, soft warm lamplight.

---

## Bugün Ne Öğrendik?


✏️ **Yazmaç:** İşlemcinin içindeki çok hızlı saklama kutusu. RISC-V'te 32 tane var (x0–x31).

0️⃣ **x0 yazmacı:** Her zaman sıfır değerindedir; yazılamaz.

🗄️ **Bellek (RAM):** Devasa raf deposu. Her göz 1 bayt (8 bit) saklar, her gözün bir adresi var.

📦 **Bayt:** 8 bit. Bellekteki en küçük adreslenebilir birim.

🥚 **Büyüğü başta / küçüğü başta** (big-endian / little-endian): Çok baytlı sayının bellekte nasıl sıralandığı. RISC-V küçüğü başta düzenini kullanır.

🍽️ **Yığıt (stack):** Tabak kulesi gibi çalışan bellek bölgesi. sp yazmacı en üstü gösterir.

🧩 **Yazmaç ataması:** Programı hazırlayan araçların yazmaçları akıllıca kullanmasına denir.

---

## Deneme Zamanı

1. RISC-V'te kaç yazmaç vardır?

2. x0 yazmacına bir sayı yazarsan ne olur?

3. Bellekte bir gözde kaç bit durur?

4. Yığıtın en üstünü hangi yazmaç gösterir?

**Yanıtlar**

1. 32 tane; x0'dan x31'e.
2. Değişmez, her zaman sıfır kalır.
3. 8 bit; buna bir bayt denir.
4. sp.
