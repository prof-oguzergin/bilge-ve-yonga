# Bir Bit Nasıl Hatırlar?

**Seri:** Bilgisayar Mimarisi Serisi — Alt Seri 4: İşlemcinin İçi — Kitap 4.4b
**Yaş grubu:** 7–10 (okuma bilen)
**Ana tema:** Geri besleme halkası, mandal, saat, flip-flop ve yazmaç: bir bit nasıl saklanır
**Karakterler:** Bilge (8 yaşında, kıvırcık kahverengi saçlı, gözlüklü kız) ve Yonga (küçük, yuvarlak, mavi-gümüş renkli sevimli robot)

---

## Sayfa 1 — Unutan Kapı

**Metin:**
Bilge masasına bir VE kapısı kurmuştu. İki düğmeye bastı, lamba yandı.

Elini çekti. Lamba hemen söndü.

{B}"Ama ben yaktım!" dedi Bilge. {B}"Neden hatırlamıyor?"

**Resim:**
Bilge'nin masası; üstünde küçük bir devre tahtası, iki düğme, pil ve bir ampul. Bilge elini düğmelerden yeni çekmiş, ampul sönük. Yüzünde şaşkın bir ifade var. Yonga masanın kenarında havada süzülüyor. Pencereden öğleden sonra ışığı giriyor.

---

## Sayfa 2 — Şimdiye Bakan Kapı

**Metin:**
{Y}"Çünkü bir kapı yalnız şu ana bakar." dedi Yonga. {Y}"Girişi ne ise çıkışı odur. Az önce ne olduğunu bilmez."

{B}"Peki bilgisayar nasıl hatırlıyor?" diye sordu Bilge.

Yonga'nın gözleri parladı. {Y}"Kapıya kendi çıkışını geri verirsen."

**Resim:**
Havada mavi bir hologram: içinde VE yazan bir kutu, soldan iki giriş oku, sağdan bir çıkış oku. Çıkış okunun ucunda küçük bir soru işareti ışığı beliriyor. Bilge hologramın yanında, kaşları kalkmış, dinliyor. Yonga elini kaldırmış anlatıyor.

---

## Sayfa 3 — Halka

**Metin:**
Yonga iki DEĞİL kapısını karşı karşıya koydu. Birincinin çıkışını ikincinin girişine, ikincinin çıkışını da birincinin girişine bağladı.

Bir halka oldu.

{Y}"Şimdi bak." dedi Yonga.

**Resim:**
Hologramda iki kutu karşılıklı duruyor; ikisinin de içinde DEĞİL yazıyor. Birinci kutunun çıkış oku ikinci kutunun girişine, ikinci kutunun çıkış oku dolanıp birinci kutunun girişine gidiyor. İki ok birlikte kapalı bir halka çiziyor. Bilge halkanın çevresinde dönen ışığı izliyor.

---

## Sayfa 4 — Kendi Kendini Besleyen

**Metin:**
Yonga halkaya bir kez "açık" verdi.

Açık, DEĞİL kapısından geçti, kapalı oldu. Kapalı, öteki kapıdan geçti, yine açık oldu. Açık geri döndü.

{B}"Kendi kendini besliyor!" dedi Bilge. {B}"Hiç durmuyor."

**Resim:**
Aynı hologram halkası, şimdi içinde ışık dönüyor: halkanın üstünde küçük parlak bir nokta, okların yönünde tur atıyor. Yanında minik bir ampul simgesi yanık duruyor. Bilge parmağıyla dönen ışığı izliyor, ağzı açık.

---

## Sayfa 5 — Yazma Anahtarı

**Metin:**
{B}"Peki içindekini nasıl değiştiririm?" diye sordu Bilge.

Yonga halkanın yanına küçük bir anahtar koydu.

{Y}"Buna **yazma anahtarı** denir." dedi. {Y}"Kapalıyken halka kendi değerini tutar. Açarsan yeni değer içeri girer."

**Resim:**
Aynı halka; solunda küçük bir anahtar var, halkaya giden bir yol açıyor. Anahtar şu an kapalı. Yolun ucunda "yeni değer" anlamında tek bir giriş oku bekliyor. Bilge anahtara uzanmış, dokunmak üzere. Yonga yanında.

---

## Sayfa 6 — Bunun Adı Mandal

**Metin:**
Bilge yazma anahtarını açtı, halkaya kapalı verdi, anahtarı yine kapattı.

Halka artık kapalı tutuyordu.

{Y}"Bu düzeneğe **mandal** denir." dedi Yonga. {Y}"Yazma anahtarı açıkken dinler, kapanınca tutar."

**Resim:**
Halkanın içindeki ışık bu kez sönük; yandaki küçük anahtar kapanmış durumda. Hologramın altında Bilge'nin elinde küçük bir mandal var, iki parmağıyla sıkıştırıp bırakıyor: bir kâğıdı tutan çamaşır mandalı gibi. Yonga gülümsüyor.

---

## Sayfa 7 — Herkes Kendi Kafasına Göre

**Metin:**
Yonga yan yana üç mandal kurdu. Yazma anahtarlarını açık bıraktı.

Değerler bir o yana bir bu yana oynadı. Biri değişirken öteki de değişti.

{B}"Karıştı!" dedi Bilge. {B}"Kim ne zaman değişecek belli değil."

**Resim:**
Hologramda yan yana üç mandal kutusu; her birinin içindeki ışık ayrı ayrı yanıp sönüyor, üçü uyumsuz. Kutuların çevresinde karışıklığı anlatan küçük dalgalı çizgiler var. Bilge iki elini şakaklarına götürmüş, gözleri kısılmış. Yonga bekliyor.

---

## Sayfa 8 — Tik

**Metin:**
Yonga havaya küçük bir sarkaç astı. Sarkaç sallandıkça bir ses çıkıyordu: tik... tik... tik...

{Y}"Buna **saat** denir." dedi Yonga. {Y}"Bilgisayarın içinde de böyle bir tik vardır. Herkes onu dinler."

**Resim:**
Hologramda altın renkli küçük bir sarkaç sağa sola sallanıyor; her salınımda çevresine bir ışık dalgası yayılıyor. Aşağıda üç mandal kutusu sıralanmış, hepsi sarkaca dönük bekliyor. Bilge sarkacın ritmine başını sallıyor. Yonga tempo tutuyor.

---

## Sayfa 9 — Hepsi Aynı Anda

**Metin:**
Tik geldi. Üç mandal aynı anda yeni değerini aldı.

Tik gelene kadar hiçbiri kımıldamadı.

{Y}"Artık kim ne zaman değişecek belli." dedi Yonga. {Y}"Saate bağlanan mandalın da yeni bir adı olur: **flip-flop**."

**Resim:**
Aynı üç kutu; sarkaçtan gelen ışık dalgası tam üçüne birden değiyor ve üçünün içindeki değer aynı anda değişiyor. Kutuların içindeki ışıklar aynı parlaklıkta. Bilge sevinçle ellerini birbirine vurmuş. Yonga ışıldıyor.

---

## Sayfa 10 — Sekizi Yan Yana

**Metin:**
Bilge sekiz flip-flopu yan yana dizdi. Hepsi aynı saati dinliyordu.

{B}"Bu artık bir bit değil." dedi. {B}"Sekiz bit."

{Y}"Ve bunun bir adı var." dedi Yonga. {Y}"**Yazmaç**. İşlemcinin elinin altındaki kutu."

**Resim:**
Hologramda sekiz mandal kutusu yan yana tek sıra hâlinde duruyor; hepsinin altından ortak bir saat çizgisi geçiyor. Kutuların içinde sırayla ışıklar yanık ve sönük. Bilge sıranın önünde durmuş, parmağıyla soldan sağa sayıyor. Yonga en sağdakini gösteriyor.

---

## Sayfa 11 — Her Tikte Bir Artan

**Metin:**
Yonga yazmacın yanına küçük bir toplayıcı taktı. Toplayıcı yazmaçtaki sayının tamamını okuyor, bir ekliyor, sonucu yine yazmacın tamamına yazıyordu. Bu her tikte bir kez oluyordu.

Bilge saydı: bir, iki, üç, dört...

{Y}"Buna **sayaç** denir." dedi Yonga. {Y}"Ne kadar ilerlediğini böyle bilir."

**Resim:**
Hologramda sekizli yazmaç sırası duruyor; sağında içinde artı olan küçük bir toplayıcı kutusu var. Yazmacın tamamından çıkan geniş bir ok toplayıcıya giriyor, toplayıcıdan çıkan yol da dolanıp sekiz hücrenin hepsine birden dönüyor: sonuç tek bir hücreye değil, yazmacın tamamına yazılıyor. Bilge sayarken parmaklarını açıyor. Yonga sarkacın altında tempo tutuyor.

---

## Sayfa 12 — Nerede Kaldığını Bilmek

**Metin:**
Bilge pencereden sokaktaki trafik lambasına baktı. Kırmızı, sonra yeşil, sonra sarı, yine kırmızı.

{B}"Lamba da hatırlıyor!" dedi. {B}"Şimdi hangi renkte olduğunu biliyor."

{Y}"İşte ona **durum** denir." dedi Yonga. {Y}"Makine, nerede kaldığını mandallarda saklar."

**Resim:**
Bilge pencerenin önünde, dışarıda bir trafik lambası duruyor; lambanın yanında havada küçük bir hologram üç daire gösteriyor: kırmızı, yeşil, sarı. Daireler bir çevrim oluşturuyor ve oklar tek yönlü: kırmızıdan yeşile, yeşilden sarıya, sarıdan yine kırmızıya. Şu anda yeşil parlıyor. Yonga Bilge'nin omzu hizasında.

---

## Sayfa 13 — Hesaplayan ve Hatırlayan

**Metin:**
{Y}"Kapılar hesaplar." dedi Yonga. {Y}"Mandallar hatırlar. Bir işlemcinin içinde ikisi de vardır."

Bilge başını salladı. {B}"Biri olmadan öteki işe yaramaz."

**Resim:**
Akşam odası. Havada iki hologram yan yana: solda içinde VE yazan bir kapı kutusu, sağda içinde küçük bir halka dönen mandal kutusu. İkisinin arasında ince bir ışık teli var. Bilge ikisine birden bakıyor, düşünceli gülümsüyor. Yonga hafifçe ışıldıyor.

---

## Sayfa 14 — Yarının Sorusu

**Metin:**
Bilge defterine sekiz küçük kutu çizdi, altına bir de sarkaç.

{B}"Bu yazmaçlar işlemcinin neresinde duruyor?" diye sordu.

Yonga göz kırptı. {Y}"Yarın onu konuşacağız."

**Resim:**
Gece odası. Bilge masasında, defterine yan yana sekiz küçük kutu ve altlarına ortak bir çizgiyle küçük bir sarkaç çizmiş; kaleminin ucu son kutunun kenarında. Masadaki küçük lamba yanıyor. Yonga defterin üstünde süzülüp çizime bakıyor. Pencerede yıldızlar var.

---

*"Kapı hesaplar, mandal hatırlar."*

## Bugün Ne Öğrendik?

🚪 Bir **kapı** yalnız şu ana bakar; girişi değişince çıkışı da değişir, eskisini hatırlamaz.

🔄 Bir kapının çıkışı kendi girişine geri verilirse bir **halka** olur; halka değerini kendi kendine tutar.

🔒 Halkanın yanına bir **yazma anahtarı** konursa **mandal** olur: anahtar açıkken dinler, kapalıyken tutar.

⏱️ **Saat**, bilgisayarın içindeki tiktir; bütün mandallar aynı anda onunla değişir.

🔁 Saate bağlanan mandala **flip-flop** denir.

📦 Sekiz flip-flop yan yana konursa bir **yazmaç** olur.

🔢 Yazmaca bir toplayıcı bağlanırsa her tikte bir artan bir **sayaç** olur.

🚦 Makinenin nerede kaldığına **durum** denir; durumu mandallar saklar.

---

## Deneme Zamanı

1. Bir kapı neden hatırlayamaz?

2. Halka nasıl kuruluyor, ne işe yarıyor?

3. Saat ne yapar?

4. Saate bağlanan mandalın adı nedir, sekiz tanesi yan yana konunca ne olur?

**Yanıtlar**

1. Yalnız şu anki girişine bakar; girişi değişince çıkışı da değişir.
2. Bir kapının çıkışı kendi girişine geri verilerek kurulur; değeri kendi kendine tutar.
3. Bütün mandalların aynı anda değişmesini sağlar; herkes tiki bekler.
4. Flip-flop denir; sekiz tanesi yan yana konunca bir yazmaç olur.
