...
function deneKapi(kap){
  ...
  ['A','B'].forEach((ad,i)=>{
    const b = document.createElement('button');
    b.type='button'; b.className='dene-kutu';
    b.innerHTML = '<span class="bit">'+ad+'</span><span class="agirlik">kapalı</span>';
    b.addEventListener('click', (e)=>{
      giris[i] = giris[i] ? 0 : 1;
      b.classList.toggle('acik', !!giris[i]);
      b.querySelector('.agirlik').textContent = giris[i] ? 'açık' : 'kapalı';
