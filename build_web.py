...
VIDEOLAR = {
    ...
    'kitap2.10-kim-daha-hizli': 'stpz2F8eFxA',
}
...
DENEME_WIDGET = {
    ...
    # 1.02a'da once kapi denemesi vardi; kapilar 1.01c'nin dersi oldugu icin
    # kaldirildi (30 Agu 2026). Bu kitabin dersi harflerin sifir bire cevrilmesi,
    # deneme de onu gosteriyor: cocuk harfe dokunur, sekiz bitlik karsiligini
    # gorur. Basamak denemesi 1.02b'de kalir, ucu de ayri ders.
    'kitap1.02a-milyarlarca-kucuk-anahtar':
        '<div class="dene" data-dene="harf"></div>',
}
...
.dene-harfler{display:flex; flex-wrap:wrap; gap:8px; justify-content:center; margin:0 0 14px}
.dene-harf{width:46px; height:46px; border-radius:12px; cursor:pointer; font-family:var(--ff-disp);
  font-weight:800; font-size:1.05rem; color:var(--kagit-solgun); background:transparent;
  border:2px solid var(--kagit-cizgi); transition:transform .12s, border-color .12s, color .12s}
.dene-harf:hover{transform:translateY(-2px)}
.dene-harf.secili{border-color:var(--parlak-koyu); color:var(--parlak-koyu); transform:translateY(-2px)}
.dene-bitler{display:flex; gap:5px; justify-content:center; flex-wrap:wrap}
.dene-bit{width:30px; height:38px; border-radius:9px; display:flex; align-items:center;
  justify-content:center; font-family:var(--ff-disp); font-weight:800; font-size:1.15rem;
  background:transparent; border:2px solid var(--kagit-cizgi); color:var(--kagit-solgun)}
.dene-bit.acik{border-color:var(--parlak-koyu); color:var(--parlak-koyu); transform:translateY(-2px)}
.dene-harf-buyuk{font-size:2.6rem}
.card .dene-not{margin:14px 0 0; max-width:none; text-align:center; font-size:.86rem;
  color:var(--kagit-solgun)}
@media (max-width:520px){ .dene-bit{width:26px; height:34px} .dene-harf{width:42px; height:42px} }
...
/* Kapi denemesinin GIRIS kutucuklari, cikistaki kutucukla ayni dile
 * getirildi (okur onerisi, 16 Agu 2026): deger yazisi sonmusken cerceveli
 * ve solgun, yanikken kehribar zeminli; giris ile cikis simetrik okunuyor.
 * Kural yalnizca .dene-girisler icine baglandi. Ayni .dene-kutu sinifi
 * basamak denemesinde de kullaniliyor; oradaki bes kutu genisletilirse
 * dar ekranda alt satira sariyor (kodda daha once duzeltilmis bir kusur).
 * Genislik burada 66'dan 92'ye cikiyor, cunku deger yazisi sayi kipinde
 * "kapali (0)" oluyor ve cerceveye sigmiyor. */
.dene-girisler .dene-kutu{width:92px; height:82px; gap:5px}
.dene-girisler .dene-kutu .agirlik{font-size:.72rem; font-weight:800; line-height:1.25;
  white-space:nowrap; opacity:1; padding:2px 9px; border-radius:999px;
  border:1.5px solid var(--kagit-cizgi); color:var(--kagit-solgun); background:transparent;
  transition:border-color .15s, color .15s, background .15s}
.dene-girisler .dene-kutu.acik .agirlik{border-color:var(--kehribar-koyu); color:#3A2400;
  background:var(--amber)}
@media (max-width:520px){ .dene-girisler .dene-kutu{width:86px; height:78px}
  .dene-girisler .dene-kutu .agirlik{font-size:.66rem; padding:2px 7px} }
...
/* Yanan sey lamba olsun. Okur, cikis hucresinin tamaminin renk degistirmesini
 * onermisti; hucreyi boyamak ayni bilgiyi dorduncu kez soylemek ve devre
 * benzetmesini bulandirmak olurdu (yanan sey lambadir, bolme degil). Bunun
 * yerine lambanin kendisine isik halesi verildi. */
.dene-lamba.yanik{filter:drop-shadow(0 0 8px var(--amber))
  drop-shadow(0 0 20px color-mix(in srgb,var(--amber) 55%,transparent))}
/* Cikis kutucugu iki durumda ayni gorunuyordu, yalnizca ampul emojisi
 * degisiyordu; okur "isigin yandigi daha belli olsun" diye bildirdi
 * (16 Agu 2026). Kutucuk artik girislerdeki .dene-kutu diliyle uyumlu:
 * sonmusken solgun cerceve, yanikken kehribar zemin. */
.card .dene-cikis{margin:.4em auto 0; max-width:none; font-family:var(--ff-disp); font-weight:800;
  font-size:1.05rem; text-align:center; color:var(--kagit-solgun);
  display:inline-block; padding:3px 14px; border:2px solid var(--kagit-cizgi);
  border-radius:999px; background:transparent;
  transition:border-color .15s, color .15s, background .15s}
.card .dene-cikis.yanik{border-color:var(--kehribar-koyu); color:#3A2400;
  background:var(--amber)}
...
function deneKur(kap){
  ...
  if(kap.dataset.dene === 'harf'){ deneHarf(kap); return; }
...
// Harf denemesi: cocuk bir harfe dokunur, harfin sekiz bitlik karsiligini
// gorur. Sayfa 9'daki "A harfi 01000001'dir" cumlesinin elle denenebilir hali.
// Harfler Ingiliz alfabesinden, kitap da oyle diyor.
function deneHarf(kap){
  // Harfler iki karakterin adindan: Bilge - YONGA. Biri kucuk biri buyuk
  // yazildi ki kucuk g ile buyuk G'nin ayri sayilar oldugu gorulsun. Aradaki
  // kisa cizginin de bir sayisi var, o da dugme.
  const HARFLER = ['B','i','l','g','e','-','Y','O','N','G','A'];
  kap.innerHTML = '<p class="dene-yonerge">Bilge ve Yonga\'nın harflerine dokun, bilgisayarın onları nasıl yazdığını gör.</p>'
    + '<div class="dene-harfler"></div>'
    + '<div class="dene-tahta">'
    +   '<div class="dene-sol">'
    +     '<p class="dene-etiket">Harf</p>'
    +     '<p class="dene-sonuc dene-harf-buyuk">A</p>'
    +   '</div>'
    +   '<div class="dene-sag">'
    +     '<p class="dene-etiket">Sekiz bit</p>'
    +     '<div class="dene-bitler"></div>'
    +   '</div>'
    + '</div>'
    + '<p class="dene-not">Küçük g ile büyük G aynı harf değil; bilgisayar ikisini ayrı sayılarla yazar.</p>';
  const sira = kap.querySelector('.dene-harfler');
  const buyuk = kap.querySelector('.dene-harf-buyuk');
  const bitler = kap.querySelector('.dene-bitler');
  const kutular = [];
  for(let i=0;i<8;i++){
    const s = document.createElement('span');
    s.className = 'dene-bit';
    s.textContent = '0';
    bitler.appendChild(s);
    kutular.push(s);
  }
  function goster(h){
    buyuk.textContent = h;
    const kod = h.charCodeAt(0);
    for(let i=0;i<8;i++){
      const b = (kod >> (7-i)) & 1;
      kutular[i].textContent = b;
      kutular[i].classList.toggle('acik', !!b);
    }
  }
  HARFLER.forEach((h,i)=>{
    const b = document.createElement('button');
    b.type = 'button'; b.className = 'dene-harf';
    b.textContent = h;
    b.addEventListener('click', (e)=>{
      sira.querySelectorAll('.dene-harf').forEach(x=>x.classList.remove('secili'));
...
function deneKapi(kap){
  ...
  function yenile(){
    ...
    cikis.classList.toggle('yanik', !!acik);
